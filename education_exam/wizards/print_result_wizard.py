    ...
    def generate_results(self):
        for exam in self.exams:
            ...
            if len(result_exam_lines)==0:
                ...
                result_exam_lines.create(data)
            result_exam_lines.calculate_exam_results
        if len(self.exams)>1:
            result_exam_lines = self.env['education.exam.result.exam.line'].search([('exam_count', '=', len(self.exams))])
            for exam in self.exams:
                result_exam_lines = result_exam_lines.search([('exam_ids','=',exam.id)])
            if len (result_exam_lines)==0:
                data = {
                    'exam_ids': [(6, 0, self.exams)],
                    'academic_year': exam.academic_year,
                    'total_working_days': exam.total_working_days,
                }
                result_exam_lines.create(data)
            result_exam_lines.calculate_average_results
